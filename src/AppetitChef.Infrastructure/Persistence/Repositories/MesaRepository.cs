...
{
    public async Task<IEnumerable<Mesa>> GetByFilialAsync(int filialId, CancellationToken ct = default) =>
        await DbSet.Include(m => m.Area).Where(m => m.Area.FilialId == filialId).ToListAsync(ct);

    public async Task<Mesa?> GetByAreaENumeroAsync(int areaId, int numero, CancellationToken ct = default) =>
